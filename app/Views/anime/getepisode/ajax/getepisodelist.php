        ...
        <div class="detail-infor-content">
            ...
            <div id="episodes-load" class="ss-list" style="display:block;">
                <?php foreach ($EpisodeList as $episode) : ?>
                    <a class="ssl-item ep-item<?= $episode->ep_id_name == $Current ? ' active' : '' ?>" title="<?= $episode->ep_name ?>" data-jname="<?= $episode->ep_jname ?>" data-number="<?= $episode->ep_id_name ?>" data-id="<?= $episode->uid ?>" onclick="handleClick(event,<?= $episode->uid ?>,<?= $episode->ep_id_name ?>)" href="/watch?anime=<?php
