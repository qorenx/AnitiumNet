('function' == typeof define && define.amd ? define : function (e, t) {
  ...
  function register(e) {
	$.post('/', e, function (e) {
	  ...
	  loading = !1,
	  e.status ? (isLoggedIn = !0, $('#modallogin').modal('hide'), $('#header_right').html(e.html), toastr.success(e.msg, '', {
		timeout: 5000
	  })
	  ) : ($('#register-error').html(e.msg), $('#register-error').show()
	  )
	})
  }
  $.get('/', function (e) {
  ...
  $('#login-form').submit(function (e) {
	e.preventDefault(),
	loading || (loading = !0, $('#login-loading').show(), $('#login-error').hide(), e = $(this).serializeArray(), $.post('/ajax/login', e, function (e) {
	  $('#login-loading').hide(),
	  loading = !1,
	  e.status ? (isLoggedIn = !0, $('#modallogin').modal('hide'), $('#header_right').html(e.html), toastr.success(e.msg, '', {
		timeout: 5000
	  })) : ($('#login-error').html(e.msg), $('#login-error').show())
	}))
  }),
  $('#register-form').submit(function (e) {
	var t;
	e.preventDefault(),
	loading || (loading = !0, $('#resister-loading').show(), $('#register-error').hide(), t = $(this).serializeArray(), 'undefined' != typeof grecaptcha ? grecaptcha.execute(recaptchaSiteKey, {
	  action: 'register'
	}).then(function (e) {
	  t.push({
		name: '_token',
  ...
  $('#forgot-form').submit(function (e) {
	e.preventDefault(),
	loading || (loading = !0, $('#loading').show(), $('#msg-error').hide(), $('#msg-success').hide(), e = $(this).serializeArray(), $.post('/ajax/forgot-password', e, function (e) {
	  $('#loading').hide(),
	  loading = !1,
	  e.status ? ($('#msg-success').html(e.msg), $('#msg-success').show()) : ($('#msg-error').html(e.msg), $('#msg-error').show())
	}))
  }),
